...
def predict_depth(im_path, output_folder):
    ...
    for t, m, s in zip(img, mean, std):
        t.sub_(m).div_(s)
    input_img = Variable(img.unsqueeze(0).type(torch.FloatTensor)).to(device)  # input----rgb图
    with torch.no_grad():
    ...
    error = torch.mean(torch.abs(reloaded_prediction - gt_depth)) * 20  
#Note that this is for illustration only. Validation pipeline will be published separately.
