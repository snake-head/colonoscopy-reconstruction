...
def predict_pose(im1_path, im2_path, output_folder):
    ...
    im1_transfer = style_transfer(im1_path).cpu().numpy()
    im2_transfer = style_transfer(im2_path).cpu().numpy()
    im1_transfer = np.transpose(im1_transfer,(1,2,0))
    im2_transfer = np.transpose(im2_transfer,(1,2,0))
    #print(im1_transfer.max())
    #print(im1_transfer.min())
    #print(im2_transfer.mean())
    #print(im2_transfer.std())
    mean = [0.705, 0.451, 0.296]
    std = [0.202, 0.161, 0.101]
    img1 = cv2.resize(np.array(im1_transfer).astype(np.float32), (int(512), int(512)), interpolation=cv2.INTER_CUBIC)
    img1 = (img1  - mean) / std
    img1 = np.transpose(img1, (2, 0, 1))
    tensor_img1 = (torch.from_numpy(img1).unsqueeze(0)).to(device)
    tensor_img1 = tensor_img1.type(torch.FloatTensor)
    img2 = cv2.resize(np.array(im2_transfer).astype(np.float32), (int(512), int(512)), interpolation=cv2.INTER_CUBIC)
    img2 = (img2- mean) / std
    #print(img1.mean())
    #print(img1.std())
    #print(img2.mean())
